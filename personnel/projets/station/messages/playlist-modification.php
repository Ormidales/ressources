...
if(isset($_GET['id']) AND !empty($_GET['id'])){
	...
	$playlistObject = $bdd->prepare('SELECT * FROM messages_playlists WHERE id = ?');
	$playlistObject->execute(array($edit_id));
	...
	if($playlistObject->rowCount() == 1){
		...
		$playlistDateTimePublication = $playlistObject['date_time_publication'];
	}
	else{
...
if (isset($_POST['playlist_nom'])) {
	if (!empty($_POST['playlist_nom'])) {
		...
		$ins = $bdd->prepare('UPDATE messages_playlists SET nom = ? WHERE id = ?');
		$ins->execute(array($playlist_nom, $playlistId));
		...
		<div class="w-full h-[100vh] overflow-hidden md:flex md:items-center block">
			...
			<div class="md:w-[89%] w-full md:h-[100vh] bg-[#151515] p-5">
				...
				<form method="post">
					...
					<input type="text" name="playlist_nom" placeholder="nom de la playlist" value="<?= $playlistNom ?>" class="border border-gray-300 w-full h-[8vh] p-3 rounded-lg"><br>
				
					<input type="submit" value="Modifier la playlist" class="px-4 py-2 bg-blue-600 hover:bg-blue-500 transition-all text-white mt-3 rounded-lg">
