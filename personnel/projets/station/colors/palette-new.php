...
if (isset($_POST['palette_nom'])) {
	if (!empty($_POST['palette_nom'])) {

		$palette_nom = htmlspecialchars($_POST['palette_nom']);

		$ins = $bdd->prepare('INSERT INTO colors_palettes (nom, date_time_publication) VALUES (?, NOW())');
		$ins->execute(array($palette_nom));
		...
		<div class="w-full h-[100vh] overflow-hidden md:flex md:items-center block">
			...
			<div class="md:w-[89%] w-full md:h-[100vh] bg-[#151515] p-5">
				...
				<form method="post">
					...
					<input type="text" name="palette_nom" placeholder="Nom de la palette" class="border border-gray-300 w-full h-[8vh] p-3 rounded-lg"><br>
				
					<input type="submit" value="Ajouter la palette" class="px-4 py-2 bg-blue-600 hover:bg-blue-500 transition-all text-white mt-3 rounded-lg">
