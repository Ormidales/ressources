...
if (isset($_POST['ressource_nom'], $_POST['ressource_lien'], $_POST['ressource_tag'])) {
	if (!empty($_POST['ressource_nom']) AND !empty($_POST['ressource_lien']) AND !empty($_POST['ressource_tag'])) {
		...
		$ins = $bdd->prepare('INSERT INTO ressources_ressources (nom, lien, tag, date_time_publication) VALUES (?, ?, ?, NOW())');
		$ins->execute(array($ressource_nom, $ressource_lien, $ressource_tag));
		...
		<div class="w-full h-[100vh] overflow-hidden md:flex md:items-center block">
			...
			<div class="md:w-[89%] w-full md:h-[100vh] bg-[#151515] p-5">
				...
				<form method="post">
					...
					<input type="text" name="ressource_nom" placeholder="nom de la ressource" class="border border-gray-300 w-full h-[8vh] p-3 rounded-lg"><br>

					<input type="text" name="ressource_lien" placeholder="lien de la ressource" class="border border-gray-300 w-full h-[8vh] p-3 rounded-lg mt-3"><br>
					...
					<input type="submit" value="Ajouter la ressource" class="px-4 py-2 bg-blue-600 hover:bg-blue-500 transition-all text-white mt-3 rounded-lg">
