...
$playlists2 = $bdd->query('SELECT * FROM ressources_playlists ORDER BY date_time_publication');

$edit_id = htmlspecialchars($_GET['id']);

if(isset($_GET['id']) AND !empty($_GET['id'])){
	
	$ressourceObject = $bdd->prepare('SELECT * FROM ressources_ressources WHERE id = ?');
	$ressourceObject->execute(array($edit_id));

	if($ressourceObject->rowCount() == 1){
		$ressourceObject = $ressourceObject->fetch();
		$ressourceId = $ressourceObject['id'];
		$ressourceNom = $ressourceObject['nom'];
		$ressourceLien = $ressourceObject['lien'];
		$ressourceTag = $ressourceObject['tag'];
		$ressourceFavoris = $ressourceObject['favoris'];
		$ressourceDatePublication = $ressourceObject['date_time_publication'];
	}
	else{
		die('Cette ressource n\'existe pas !');
	}
}
else{
	die('Erreur');
...
if (isset($_POST['ressource_nom'], $_POST['ressource_lien'], $_POST['ressource_tag'], $_POST['ressource_favoris'])) {
	if (!empty($_POST['ressource_nom']) AND !empty($_POST['ressource_lien']) AND !empty($_POST['ressource_tag']) AND !empty($_POST['ressource_favoris'])) {

		$ressource_nom = htmlspecialchars($_POST['ressource_nom']);
		$ressource_lien = htmlspecialchars($_POST['ressource_lien']);
		$ressource_tag = htmlspecialchars($_POST['ressource_tag']);
		$ressource_favoris = htmlspecialchars($_POST['ressource_favoris']);

		$ins = $bdd->prepare('UPDATE ressources_ressources SET nom = ?, lien = ?, tag = ?, favoris = ? WHERE id = ?');
		$ins->execute(array($ressource_nom, $ressource_lien, $ressource_tag, $ressource_favoris, $ressourceId));
		...
		<div class="w-full h-[100vh] overflow-hidden md:flex md:items-center block">
			...
			<div class="md:w-[89%] w-full md:h-[100vh] bg-[#151515] p-5">
				...
				<form method="post">
					...
					<input type="text" name="ressource_nom" placeholder="nom de la ressource" value="<?= $ressourceNom ?>" class="border border-gray-300 w-full h-[8vh] p-3 rounded-lg"><br>

					<input type="text" name="ressource_lien" placeholder="lien de la ressource" value="<?= $ressourceLien ?>" class="border border-gray-300 w-full h-[8vh] p-3 rounded-lg mt-3"><br>

					<select name="ressource_tag" class="border border-gray-300 w-full h-[8vh] p-3 rounded-lg mt-3">
						<?php while($playlist2 = $playlists2->fetch()) { ?>
					    <option value="<?= $playlist2['nom'] ?>"><?= $playlist2['nom'] ?></option>
					    <?php } ?>
					</select><br>

					<select name="ressource_favoris" class="border border-gray-300 w-full h-[8vh] p-3 rounded-lg mt-3">
					    <option value="oui">Ressource favorite - Oui</option>
					    <option value="non">Ressource favorite - Non</option>
					</select><br>
				
					<input type="submit" value="Modifier la ressource" class="px-4 py-2 bg-blue-600 hover:bg-blue-500 transition-all text-white mt-3 rounded-lg">
