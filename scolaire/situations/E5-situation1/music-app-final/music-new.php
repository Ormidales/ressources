...
<body style="font-family: Source Code Pro;" class="p-8 bg-[#151515]">
	...
	<form method="post">
		...
		<input type="text" name="music_titre" placeholder="Titre de la musique" class="border border-gray-300 w-[50vh] h-[8vh] p-3 rounded-lg"><br>

		<select name="music_playlist" class="border border-gray-300 w-[50vh] h-[8vh] p-3 mt-3 rounded-lg">
			<?php while($b = $playlist->fetch()) { ?>
		    <option value="<?= $b['libelle'] ?>"><?= $b['nom'] ?></option>
			<?php } ?>
		</select><br>

		<textarea name="music_link" placeholder="Lien vers la musique" class="border border-gray-300 w-[50vh] h-[15vh] mt-3 p-3 rounded-lg"></textarea><br>

		<input type="submit" value="Ajouter la musique" class="px-4 py-2 bg-blue-600 hover:bg-blue-500 transition-all text-white mt-3 rounded-lg">
